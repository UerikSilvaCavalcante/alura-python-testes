...
class TestOrderUserIntegration:
    ...
    def test_user_order_valid_user_with_services(
        ...
        order_data = {"user_email": sample_user["email"], **sample_items}

        response = requests.post(
            f"{base_urls['order']}/create", json=order_data, allow_redirects=False
        )
        assert response.status_code == 302

    @patch("requests.post")
    def test_create_order_valid_user_mocked(
        self, mock_post, base_urls, sample_user, sample_items
    ):
    ...
    def test_create_order_user_not_found_with_services(
        self, base_urls, sample_items, skip_if_services_down
    ):
        """Testa a criação de um pedido com usuário não encontrado"""
        skip_if_services_down("user", "order")

        order_data = {"user_email": "naoexite@teste.com ", **sample_items}
        response = requests.post(
            f"{base_urls['order']}/create", json=order_data, allow_redirects=False
        )
        assert response.status_code in [
            404,
            400,
        ]  # Espera erro 404 ou 400 se usuário não encontrado
